...
async function loadLive(){
  ...
  boardEl.innerHTML = rows.length ? rows.map((p,i)=>`
    ...
    : '<div class="empty-note">Nobody\'s free this period — check back at the next bell.</div>';
}
  ...
  search.addEventListener('input', ()=>{
    ...
    debounce = setTimeout(async ()=>{
      ...
      results.innerHTML = users.map(u=>`
        ...
        </div>`).join('') || '<div class="empty-note">No one found. Try a different username or check the spelling.</div>';
...
async function renderTogetherResults(){
  ...
  el.innerHTML = any ? html : '<div class="empty-note">No common free period this week — try a different pair of people.</div>';
}
